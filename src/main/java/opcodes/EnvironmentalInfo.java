...
public class EnvironmentalInfo {
    ...
    private void populateCodeList() {
        allCodes.add(new Opcode("30", "ADDRESS", "Get address of currently executing account", 2, false));
        allCodes.add(new Opcode("31", "BALANCE", "Get balance of the given account", 20, false));
        allCodes.add(new Opcode("32", "ORIGIN", "Get execution origination address", 2, false));
        allCodes.add(new Opcode("33", "CALLER", "Get caller address", 2, false));
        allCodes.add(new Opcode("34", "CALLVALUE", "Get deposited value by the instruction/transaction responsible for this execution", 2, false));
        allCodes.add(new Opcode("35", "CALLDATALOAD", "Get input data of current environment", 3, false));
        allCodes.add(new Opcode("36", "CALLDATASIZE", "Get size of input data in current environment", 2, false));
        allCodes.add(new Opcode("37", "CALLDATACOPY", "Copy input data in current environment to memory", 3, false));
        allCodes.add(new Opcode("38", "CODESIZE", "Get size of code running in current environment", 2, false));
        allCodes.add(new Opcode("39", "CODECOPY", "Copy code running in current environment to memory", 3, false));
        allCodes.add(new Opcode("3a", "GASPRICE", "Get price of gas in current environment", 2, false));
        allCodes.add(new Opcode("3b", "EXTCODESIZE", "Get size of an account's code", 20, false));
        allCodes.add(new Opcode("3c", "EXTCODECOPY", "Copy an account's code to memory", 20, false));
